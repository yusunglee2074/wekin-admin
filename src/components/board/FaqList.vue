        ...
        <div class="box">
          <div class="box-header">
            <router-link to="/faq/add">
              <button class="btn btn-primary pull-right">새 질문 작성</button>
            </router-link>
                  ...
                  <table role="grid" id="data_table" class="table table-bordered table-striped dataTable">
                    ...
                    <tbody>
                      <tr class="even" role="row" v-for="(item, key, index) in items">
                        ...
                        <td><router-link :to="'/faq/' + item.board_key"><button class="btn btn-primary">수정</button></router-link></td>
...
export default {
  ...
  methods: {
    fetchData () {
      this.$http.get('/board/faq/')
      .then(res => {
        this.items = res.data
        return this.$http.get('/env/faq/category')
      })
      .then(res => {
    ...
    categoryRemove (item) {
      ...
      if (result) {
        this.$http.delete('/env/faq/category/' + item.env_key)
        .then(val => {
    ...
    categoryEdit (key) {
      this.$http.put('/env/faq/category', this.cate)
      .then(_ => this.$http.get('/env/faq/category'))
      .then(val => {
    ...
    commitCategory () {
      this.$http.post('/env/faq/category', this.cate)
      .then(_ => this.$http.get('/env/faq/category'))
      .then(val => {
    ...
    deleteItem (item) {
      ...
      if (flag) {
        this.$http.delete('/board/faq/' + item.board_key)
